# Blog Site Template (parc PBN GEO perso)

Template du parc PBN GEO perso de Damien, compte GitHub `DamienBo59`, domaines chez o2switch. Aucun lien avec le parc datashake. Doctrine et roots : lire le `CLAUDE.md`.
...
1. Dupliquer ce dossier vers `~/code/sites/pbn-perso/pbn-geo-<nom>`
2. Ouvrir le nouveau dossier dans Claude Code
...
- **GitHub Pages** : hebergement gratuit, compte `DamienBo59`
...
| `/create-article-seo` | **Methode 2**, production evergreen SEO bilingue FR+EN en batch local. 3 modes de selection des mots-cles (roadmap du blog, roadmap externe, KW a la demande) et 3 strategies de scheduling. Maillage cross-batch. Voir le `CLAUDE.md`. |
| `/create-article-auto` | **Methode 1**, publication evergreen SEO totalement automatique depuis `roadmap.yaml`, sans input humain. Concue pour une routine planifiee. Aucune routine n'est encore montee en perso. |
...
.claude/
├── scripts/
│   └── fetch-image.sh       ← Image libre de droit auto (API Openverse)
├── settings.json            ← Permissions Claude Code du template
├── skills/
│   ├── create-site/         ← Workflow creation de site complet
│   ├── create-article-geo/  ← Workflow creation d'article (multi-types)
│   ├── create-article-seo/  ← Methode 2 : batch local evergreen SEO
│   ├── create-article-auto/ ← Methode 1 : publication auto depuis la roadmap
